...
Republish is **at-least-once** on the server. Handlers must be idempotent, or
callers opt into fail-fast.
...
already hold is the one client-visible completion. The server may see the
request twice if the original delivery already ran — that is at-least-once,
not exactly-once.
