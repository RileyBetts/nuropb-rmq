...
run_python() {
  local kind="$1"
  uv run python -m bench.remeasure_io --kind "$kind"
}
...
run_lean_plain() {
  ...
  run_lean_cell rpc 200 64 "nr.bench.lean.${tag}.rpc"
  run_lean_cell rpc_overlap 400 64 "nr.bench.lean.${tag}.ov"
}
...
if [[ "$DO_PLAIN" -eq 1 ]]; then
  ...
  for i in $(seq 1 "$PASSES"); do
    echo "===== PYTHON PLAIN pass ${i}/${PASSES} (${HOST}:${PLAIN_PORT}) ====="
    run_python plain
    echo "===== LEAN PLAIN pass ${i}/${PASSES} (${HOST}:${PLAIN_PORT}) ====="
    run_lean_plain "p${i}"
  done
fi
...
if [[ "$DO_AMQPS" -eq 1 ]]; then
  ...
  for i in $(seq 1 "$PASSES"); do
    echo "===== PYTHON AMQPS pass ${i}/${PASSES} (${HOST}:${AMQPS_PORT}) ====="
    run_python amqps
    echo "===== LEAN AMQPS pass ${i}/${PASSES} (${HOST}:${AMQPS_PORT}) ====="
    run_lean_amqps "t${i}"
  done
fi
