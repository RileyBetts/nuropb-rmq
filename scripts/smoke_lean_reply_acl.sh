...
# Lean live reply-forge 403. Needs RabbitMQ management API (15672) + lake.
set -euo pipefail
...
# Admin for management API + victim declare (not the restricted ACL users).
...
source "$ROOT/scripts/setup_reply_acl_users.sh"
...
if [[ "$out" != *"reply-acl: forge denied 403"* || "$out" != *"reply-acl: service publish ok"* ]]; then
  echo "FAIL lean_reply_acl" >&2
  exit 1
fi
echo "PASS lean_reply_acl"
