...
# Create management-API users matching test_reply_acl_amqp.py and export
# NUROPB_RMQ_ACL_* for the Lean reply-forge 403 smoke.
set -euo pipefail
...
CLIENT="nr.acl.client.${suffix}"
SVC="nr.acl.svc.${suffix}"
...
mgmt PUT "/api/permissions/%2F/${CLIENT}" \
  '{"configure":"^nr\\.reply\\.","write":"^nr\\.mesh","read":"^nr\\.reply\\."}'
mgmt PUT "/api/permissions/%2F/${SVC}" \
  '{"configure":"^nr\\.reply\\.","write":"^nr\\.mesh.*|^nr\\.reply\\..*|^nr\\.dlx\\..*|^amq\\.default$","read":"^nr\\.reply\\."}'
...
echo "ACL users ${CLIENT} / ${SVC} victim ${VICTIM}"
