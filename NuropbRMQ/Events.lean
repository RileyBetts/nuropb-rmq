...
def EventPublisher.start (cfg : ConnectionConfig) (exchange : String)
    (exchangeType : String := "topic") : IO EventPublisher := do
  let c ← connect cfg
  let _ ← openChannel c 1
  exchangeDeclare c 1 exchange exchangeType (autoDelete := true)
  return { conn := c, exchange, exchangeType }

def EventPublisher.close (p : EventPublisher) : IO Unit :=
  NuropbRMQ.close p.conn

def EventPublisher.publish (p : EventPublisher) (routingKey method : String) (params : Json) : IO Unit := do
  let key := if p.exchangeType == "fanout" then "" else routingKey
  let body := encodeNotification method (some params)
  basicPublish p.conn p.channelId body p.exchange key { contentType := some "application/json" }
...
def EventSubscriber.start (cfg : ConnectionConfig) (exchange : String)
    (exchangeType : String := "topic") (bindingKey : String := "#") : IO EventSubscriber := do
  let c ← connect cfg
  let _ ← openChannel c 1
  exchangeDeclare c 1 exchange exchangeType (autoDelete := true)
...
def EventSubscriber.close (s : EventSubscriber) : IO Unit :=
  NuropbRMQ.close s.conn

def EventSubscriber.receive (s : EventSubscriber) (timeoutMs : Nat := 5000) : IO (String × Json) := do
  let msg ← NuropbRMQ.receive s.conn timeoutMs
  basicAck s.conn s.channelId msg.deliveryTag
